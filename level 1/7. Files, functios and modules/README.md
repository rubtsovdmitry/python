...
# Напишите программу, которая будет считывать содержимое файла, добавлять к считанным строкам порядковый номер и выводить их в таком виде на экран. Каждая строка должна начинаться с ее номера, двоеточия и пробела, после чего должен идти текст строки из исходного файла.
...
# Для обработки можно взять файл из репозитория "pushkin.txt".
...
# Напишите программу, которая будет искать слово в тексте, которое повторяется больше всего раз. При этом пробелы, знаки препинания и цифры должны быть проигнорированы. Также не должен учитываться регистр, то есть символы a и A должны восприниматься как одна буква. 
# Имя файла для анализа пользователь должен передавать программе посредством аргумента командной строки. Если программе не удастся открыть файл для анализа или аргументов командной строки будет слишком много, на экране должно быть отображено соответствующее сообщение об ошибке.
# Для обработки можно взять файл из репозитория "book.txt".
...
# Файл для обработки можно найти в репозитории.

#############################

# 1.7.50.task.py
# Условие:
# Случайный пароль из двух слов.
...
# Только в примере я буду обрабатывать не английские буквы, а русские. Предлагаю воспользоваться готовым файлом, который находится в репозитории и называется pushkin.txt (это Сказка о мертвой царевне и о семи богатырях).
...
# Предлагаю воспользоваться готовым файлом, который находится в репозитории и называется pushkin.txt (это Сказка о мертвой царевне и о семи богатырях).
...
# В данном упражнении вам предстоит написать программу для определения наличия дублей слов в тексте. При нахождении повтора на экран должен выводиться номер строки. Удостоверьтесь, что программа корректно обрабатывает случаи, когда повторяющиеся слова находятся на разных строках, как в предыдущем примере.
# Для проверки можно взять подготовленный файл с ошибками из репозитория под названием: "pushkin_mistakes.txt".
...
# Есть текст: "Позвони мне завтра по номеру 415-555-1011. 415-555-9999 - это номер телефона моего офиса."
# Программа должна найти в нём номера телефонов и показать на экране.
