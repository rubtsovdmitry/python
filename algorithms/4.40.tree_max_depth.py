...
def main(node):
    if len(node["children"]) > 0:                                                   # это рекурсивный случай, в узле котором есть дочерние элементы
        for child in node["children"]:
            depth = main(child)
        return depth + 1                                                            # при разборке стека, каждый уровень глубины увеличивается на 1. На вершине стека, когда все ветви дерева соберутся обратно в главный узел, будет определена максимальная глубина дерева.
    else:                                                                           # Базовый случай - конец ветки или стек доехал до листа. Начинается подсчёт глубины дерева с базового случая, который будет 1
        return 1
