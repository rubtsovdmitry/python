...
# 4.37.task.py
# Класс "Стек" (продолжение, программа "Теги в HTML").
...
# Нужно написать два алгоритма для прямого и обратного обхода дерева с выводом данных для каждого узла.   
# Смотреть условие программы с иллюстрациями: https://python.rubtsovdmitry.ru/content/4_algorithms/recursion_22_tree_traversal.html

##########################################################

# 4.39.task.py
# Алгоритм поиска с возвратом и обхода дерева. Имя из 8 букв.
# Нужно создать структуру данных к дереву.
# И написать два варианта программы:
...
# Смотреть условие программы с деревом и иллюстрациями: https://python.rubtsovdmitry.ru/content/4_algorithms/recursion_23_tree_8_letter.html
...
# Смотреть условие программы с деревом и иллюстрациями: https://python.rubtsovdmitry.ru/content/4_algorithms/recursion_24_tree_max_depth.html
...
# 4.42.task.py
# Графы. Поиск в ширину (повторение). Теперь только с самописным классом FIFO.  
# Дан словарь (хеш-таблица): a = {"Девочка": ["Вася", "Петя"], "Вася": ["Семён", "Татьяна"], "Петя": ["Снежана"], "Семён": [], "Татьяна": ["Галя"], "Снежана": ["Катя", "Женя"], "Галя": ["Игорь"], "Катя": ["Человек-паук"], "Женя": ["Джон"], "Игорь": ["Железный человек"], "Железный человек": ["Человек-паук"]}.
# Допустим, что источник - это "Девочка". Ключ словаря носит имя какого-то человека, а значения ключа - это его знакомые. Нужно найти минимальный путь от "Девочки" к "Человеку-пауку" с помощью графов.
# В данном варианте программы нужно написать класс для FIFO. Брать его из коллекции (<from collections import deque>) нельзя.
